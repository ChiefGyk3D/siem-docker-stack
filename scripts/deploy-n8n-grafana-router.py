        ...
        "nodes": [
            ...
            },
            {
                "parameters": {
                    ...
                    "jsonBody": '={\n  "content": "' + DISCORD_MENTION + '",\n  "embeds": [{\n    "title": "🔥 FIRING — {{ ($json.body.alerts || [{}])[0].labels?.alertname || \'Grafana Alert\' }}",\n    "description": "{{ ($json.body.alerts || [{}])[0].annotations?.summary || \'Alert triggered\' }}",\n    "color": 15158332,\n    "fields": [\n      { "name": "Severity", "value": "{{ ($json.body.alerts || [{}])[0].labels?.severity || \'unknown\' }}", "inline": true },\n      { "name": "Source", "value": "{{ ($json.body.alerts || [{}])[0].labels?.source || \'unknown\' }}", "inline": true },\n      { "name": "Alert Group", "value": "{{ ($json.body.alerts || [{}])[0].labels?.alert_group || \'N/A\' }}", "inline": true },\n      { "name": "Grafana Folder", "value": "{{ ($json.body.alerts || [{}])[0].labels?.grafana_folder || \'N/A\' }}", "inline": true },\n      { "name": "Starts At", "value": "{{ ($json.body.alerts || [{}])[0].startsAt || \'N/A\' }}", "inline": false },\n      { "name": "Details", "value": "{{ (($json.body.alerts || [{}])[0].annotations?.description || \'N/A\').slice(0, 950) }}", "inline": false },\n      { "name": "Dashboard / Panel", "value": "{{ ($json.body.alerts || [{}])[0].dashboardURL || \'N/A\' }}\\n{{ ($json.body.alerts || [{}])[0].panelURL || \'\' }}", "inline": false },\n      { "name": "Labels (raw)", "value": "```json\\n{{ JSON.stringify((($json.body.alerts || [{}])[0].labels || {}), null, 2).slice(0, 900) }}\\n```", "inline": false }\n    ],\n    "timestamp": "{{ ($json.body.alerts || [{}])[0].startsAt || new Date().toISOString() }}"\n  }]\n}',
                    "options": {},
                },
